...
def _create_input_sequences(input_data, tw, output_window, diff, mean_std):
    ...
    for i in range(L-tw):
        ...
        train_seq = intput_seq[1:]
        if diff:
            # diff_seq = np.append(np.diff(intput_seq), np.array([SEP]))
            if not diff_scaler:
                diff_scaler = StandardScaler()
                diff_seq = np.append(diff_scaler.fit_transform(np.diff(intput_seq).reshape(-1, 1)), np.array([SEP]))
            else:
                diff_seq = np.append(diff_scaler.transform(np.diff(intput_seq).reshape(-1, 1)), np.array([SEP]))
            train_seq = np.append(diff_seq, train_seq)
        
        if mean_std:
            mean_std_val = np.array([np.mean(train_seq), SEP, np.std(train_seq)])
...
def get_data(df, input_window, output_window, scaler_name=None, diff=False, mean_std=False):
    ...
    return train_sequence, test_data, (scaler, diff_scaler)
